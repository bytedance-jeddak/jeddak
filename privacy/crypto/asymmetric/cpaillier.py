...
from privacy.crypto.asymmetric.a_cryptosystem import AsymmetricCryptosystem
...
class PaillierPrivateKey(object):
    ...
    def raw_decrypt(self, ciphertext):
        ...
        decrypt_to_p = self.l_function(IntegerArithmetic.powmod(ciphertext, self.p-1, self.psquare), self.p) * self.hp % self.p
        decrypt_to_q = self.l_function(IntegerArithmetic.powmod(ciphertext, self.q-1, self.qsquare), self.q) * self.hq % self.q
    ...
    def h_function(self, x, xsquare):
        ...
        """
        return IntegerArithmetic.invert(self.l_function(IntegerArithmetic.powmod(self.public_key.g, x - 1, xsquare),x), x)
